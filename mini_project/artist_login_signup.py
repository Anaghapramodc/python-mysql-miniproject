    ...
    if e==gmail and pw==pwd:
          ...
          for i in myresult:
              ...
              if gmail==dict.get("gmail") and pwd==dict.get("password"):
                  mycursor.execute("use art_gallary")
                  sql = "insert into artist_sell(gmail,password,painting_name,mediam,size,price,art_code,sell_or_not,user_name) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                  val = [
...
def login():
    ...
    myresult = mycursor.fetchall()
    # print(myresult)
    d = {1: 'id_no', 2: 'gmail', 3: 'password', 4: 'painting_name', 5: 'mediam', 6: 'size', 7: 'price',
